...
export const useSocialAuth = () => {
  ...
  const initSocialLogin = ({ socialType }: { socialType: SocialType }) => {
    ...
    const socialLoginUrls = {
      ...
      apple: `https://appleid.apple.com/auth/authorize?client_id=${import.meta.env.VITE_APPLE_CLIENT_ID}&redirect_uri=https://api.fittner.co.kr/api/v1/auth/apple/test&response_type=code&response_mode=form_post&scope=email&state=${socialType}`
    };
